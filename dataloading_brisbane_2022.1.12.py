...
class Fusion_VPR_Dataset(object):
    def __init__(self, object, count, ignore, combine, use='v', input_transform = input_transform()):
        ...
        self.use = use
        self.transform_rgb = input_transform_rgb
        self.transform_event = input_transform_event
        self.nNeg = 5
    ...
    def get_triplet_slice(self, idx, idy, neg_idx, neg_idy, idt):
        ...
        for j in range(len(neg_idy)):
            if j == 0:
                if idt == 'frames':
                    neg_out = self.transform_rgb(Image.fromarray(neg_list[j* (self.lenth-1) + neg_idy[j]])).unsqueeze(0)
                else:
                    neg_out = self.transform_event(Image.fromarray(neg_list[j* (self.lenth-1) + neg_idy[j]])).unsqueeze(0)  
            else:
                if idt == 'frames':
                    neg_out = torch.cat((neg_out, self.transform_rgb(Image.fromarray(neg_list[j* (self.lenth-1) + neg_idy[j]])).unsqueeze(0)), axis=0)
                else:
                    neg_out = torch.cat((neg_out, self.transform_event(Image.fromarray(neg_list[j* (self.lenth-1) + neg_idy[j]])).unsqueeze(0)), axis=0)
        if idt == 'frames':        
            return self.transform_rgb(Image.fromarray(return_list[self.mapping[idy][0]])), self.transform_rgb(Image.fromarray(return_list[self.mapping[idy][1]])), neg_out
        else:
            return self.transform_event(Image.fromarray(return_list[self.mapping[idy][0]])), self.transform_event(Image.fromarray(return_list[self.mapping[idy][1]])), neg_out
...
def Generate_from_BrisbanVPR(dataset_folder, query_traverse, reference_traverse, ignore, combine=True, use='v'):
    ...
    whole_dataset = Fusion_VPR_Dataset(images_all_combined_set2, count, ignore, combine, use)
...
if __name__ == "__main__":
    ...
    reference_traverse = ['dvs_vpr_2020-04-29-06-20-23', 'dvs_vpr_2020-04-28-09-14-11', 'dvs_vpr_2020-04-22-17-24-21', 'dvs_vpr_2020-04-21-17-03-03']
    #reference_traverse = ['dvs_vpr_2020-04-29-06-20-23', 'dvs_vpr_2020-04-28-09-14-11', 'dvs_vpr_2020-04-27-18-13-29', 'dvs_vpr_2020-04-22-17-24-21', 'dvs_vpr_2020-04-21-17-03-03']
    whole_dataset = Generate_from_BrisbanVPR(dataset_folder, query_traverse, reference_traverse, 'reconstruction')
    batch_size = 4
    num_workers = 1
    train_loader = DataLoader(dataset=whole_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, drop_last=False)
    '''
    
    '''
    reference_traverse = 'dvs_vpr_2020-04-22-17-24-21'
    images_all_combined_set1, images_all_combined_set2 = get_test_dataset(dataset_folder, query_traverse, reference_traverse)
    for i in range(len(images_all_combined_set2['reconstruction/events'])):
        print(images_all_combined_set2['reconstruction/events'][i])
    '''
    dataset_folder = '/mnt/data/Event-VPR/'
